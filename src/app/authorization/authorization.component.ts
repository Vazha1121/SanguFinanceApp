...
export class AuthorizationComponent {
  ...
  onRegister() {
    const { fullName, email, password, confirmPassword, currencyPreference } = this.RegisterForm.value;
  
    if (password !== confirmPassword) {
      return;
    }
  
    this.authService.register(fullName, email, password, confirmPassword, currencyPreference).subscribe({
      next: (response) => {
